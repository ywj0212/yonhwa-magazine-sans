# Third-Party Notices — Yonhwa Magazine Sans (25w51a)

This distribution includes or is derived from the following third-party font software.
All listed fonts are distributed under the SIL Open Font License, Version 1.1 (OFL-1.1).
See `LICENSE` for the full license text.

This project does not use upstream Reserved Font Names as the primary font name.

---

## Lato
Copyright: Lukasz Dziedzic (typoland)
License: SIL Open Font License, Version 1.1 (OFL-1.1)
Source: https://fonts.google.com/specimen/Lato

---

## Noto Sans CJK JP (Noto Sans CJK Japanese)
Copyright: Google
License: SIL Open Font License, Version 1.1 (OFL-1.1)
Source: https://github.com/googlefonts/noto-cjk

---

## Gmarket Sans
Copyright: Gmarket
License: SIL Open Font License, Version 1.1 (OFL-1.1)
Source: https://corp.gmarket.com/fonts

---

## Pretendard
Copyright: Pretendard Project Authors
License: SIL Open Font License, Version 1.1 (OFL-1.1)
Source: https://github.com/orioncactus/pretendard
